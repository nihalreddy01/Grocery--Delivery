...
export const createOrder = async (req: Request,res: Response)=>{
    ...
    //Send stock update events for each 
    for(const item of orderItems){
...
export const getUserOrders = async (req: Request,res: Response)=>{
    ...
    const orders = await prisma.order.findMany({
        where,
        include: {deliveryPartner: {select: {name: true, phone: true,email:true}}},
...
export const getOrderLocation = async (req: Request,res: Response)=>{
    const order = await prisma.order.findFirst({where: {id: req.params.id as string, userId: req.user!.id}, select: {liveLocation: true,status: true}})
